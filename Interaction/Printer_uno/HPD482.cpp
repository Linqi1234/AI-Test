	...
	for(pixlin=0;pixlin<vlines;(pixlin+=sizewidth))//行扫描并打印一行
	{
		...
		for(sa=0;sa<strnum;(sa+=sizewidth))//字符串扫描并发送数据
		{
			...
			for(si=0;si<sizewidth;si++)
			{
				...
				#endif
        #if IS_ASCII18_EN 
				if(sizew==9)onepixline[sa+si]=pgm_read_byte(&ascii_1809[offset][pixlin+si]);
				#endif
        #if IS_ASCII29_EN 
				if(sizew==14)onepixline[sa+si]=pgm_read_byte(&ascii_2914[offset][pixlin+si]);
				#endif
        #if IS_ASCII34_EN 
				if(sizew==17)onepixline[sa+si]=pgm_read_byte(&ascii_3417[offset][pixlin+si]);
				#endif
...
/*重写一个打印大数字的方法*/
void HPD482::Print_String_big(u8 *onestr,u8 strnum,int sizeh,int sizew)
{
	int sizewidth;
	int pixlin;
	int vlines;
	...
	for(pixlin=0;pixlin<vlines;(pixlin+=sizewidth))//行扫描并打印一行
	{
      ...
      if(sizew == 48){
        if (*pronestr ==32){
          ...
          offset = *pronestr - 47; //得到偏移量
        }
         

      }else if (sizew == 51) {
        offset = *pronestr - 49; //得到偏移量
      } else { 
        if (*pronestr ==32){
           offset = 0;
        }else if (*pronestr < 97) {
          offset = *pronestr - 65 + 27; //得到偏移量
        } else {
          offset = *pronestr - 96; //得到偏移量
        }
      }
			...
			for(si=0;si<sizewidth;si++)
			{
				...
				#endif
        #if IS_ASCII27_EN 
				if(sizew==13)onepixline[sa+si]=pgm_read_byte(&ascii_2713[offset][pixlin+si]);
				#endif
        #if IS_ASCII29_EN 
				if(sizew==14)onepixline[sa+si]=pgm_read_byte(&ascii_2914[offset][pixlin+si]);
				#endif
				...
				#endif
         #if IS_ASCII96_EN 
				if(sizew==48)onepixline[sa+si]=pgm_read_byte(&ascii_9648[offset][pixlin+si]);
				#endif
       #if IS_ASCII103_EN 
				if(sizew==51)onepixline[sa+si]=pgm_read_byte(&ascii_10351[offset][pixlin+si]);
				#endif
			}
			pronestr++;//指向下一字符
		}
